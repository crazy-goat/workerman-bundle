...
- Replace `testBootstrapClosesProcOpenPipes` (which read `bootstrap.php` as a string and asserted on substrings) with a behavioral test that exercises the `proc_open` pipe cleanup pattern and asserts all pipe resources are closed after `fclose()` ([#319](https://github.com/crazy-goat/workerman-bundle/issues/319))
