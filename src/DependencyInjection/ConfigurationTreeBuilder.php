...
final readonly class ConfigurationTreeBuilder
{
    ...
    private function configureTopLevelPrimitiveNodes(NodeBuilder $node): void
    {
        $node
            ...
            ->integerNode('connection_timeout')
                ->info('Max seconds to wait for a complete request before closing the connection. Protects against slowloris-style attacks where a client sends data very slowly. Set to 0 to disable the timeout.')
                ->min(0)
                ->defaultValue(120)
                ->end()
            ->integerNode('keepalive_timeout')
                ->info('Max seconds to keep an idle keep-alive connection open before closing it. Set to 0 to disable the timeout.')
                ->min(0)
                ->defaultValue(30)
                ->end()
    ...
    private function configureServersNode(NodeBuilder $node): void
    {
        $node
            ->arrayNode('servers')
                ->prototype('array')
                    ...
                    ->children()
                        ...
                        ->booleanNode('reuse_port')
                            ...
                            ->defaultFalse()
                            ->end()
                        ->integerNode('body_size_cap')
                            ->info('Per-server max request body size in bytes. Overrides the global max_package_size for this server. Useful to tighten limits on specific endpoints.')
                            ->min(1)
                            ->defaultNull()
                            ->end()
                        ->booleanNode('serve_files')
                            ->info('Deprecated boolean switch for the legacy static file serving path configured via root_dir. Configure a StaticFilesMiddleware service instead.')
                            ->defaultFalse()
                            ->setDeprecated('workerman', '0.9.3', 'Will be removed in 1.0. Use the StaticFilesMiddleware instead. See docs/security.md#static-files-protection. The static_files.allowed_extensions allowlist no longer applies once you move to the middleware; configure the $allowedExtensions constructor argument instead.')
                            ->end()
                        ->scalarNode('root_dir')
                            ->info('Deprecated path to the public directory served by the legacy serve_files static file serving path. Configure a StaticFilesMiddleware service instead.')
                            ->setDeprecated('workerman', '0.9.3', 'Will be removed in 1.0. Use the StaticFilesMiddleware instead. See docs/security.md#static-files-protection. The static_files.allowed_extensions allowlist no longer applies once you move to the middleware; configure the $allowedExtensions constructor argument instead.')
                            ->defaultNull()
                        ...
                        ->arrayNode('static_files')
                            ->info('Security configuration for the deprecated serve_files/root_dir static file serving path. A StaticFilesMiddleware registered as a service takes $allowedExtensions as a constructor argument instead.')
                            ->setDeprecated('workerman', '0.9.3', 'Will be removed in 1.0. Configure a StaticFilesMiddleware service with its $allowedExtensions constructor argument instead. See docs/security.md#static-files-protection. The static_files allowlist no longer applies once you use the middleware.')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->arrayNode('allowed_extensions')
                                    ->info('List of allowed file extensions without leading dot (e.g. [css, js, png, jpg, svg, woff2, ico]). When set, only files with these extensions are served. Leave empty to serve any file.')
                                    ->prototype('scalar')->end()
                                    ->defaultValue([])
                                    ->end()
                                ->end()
    ...
    private function configureReloadStrategyNode(NodeBuilder $node): void
    {
        $reloadNode = $node->arrayNode('reload_strategy')
            ->info('Reload strategies configuration')
            ->addDefaultsIfNotSet();

        $children = $reloadNode->children();

        $this->configureExceptionStrategy($children);
        $this->configureMaxRequestsStrategy($children);
        $this->configureFileMonitorStrategy($children);
        $this->configureAlwaysStrategy($children);
        $this->configureMemoryStrategy($children);

        $children->end();
        $reloadNode->end();
    }

    private function configureExceptionStrategy(NodeBuilder $node): void
    {
        $node
            ->arrayNode('exception')
    ...
    private function configureMaxRequestsStrategy(NodeBuilder $node): void
    {
        $node
    ...
    private function configureFileMonitorStrategy(NodeBuilder $node): void
    {
        $node
    ...
    private function configureAlwaysStrategy(NodeBuilder $node): void
    {
        $node
            ->arrayNode('always')
    ...
    private function configureMemoryStrategy(NodeBuilder $node): void
    {
        $node
            ->arrayNode('memory')
                ...
                ->children()
                    ...
                    ->end()
                    ->integerNode('limit')
                        ->info('Maximum emalloc\'ed memory (memory_get_usage(), not real usage) after which the worker will be reloaded')
                        ->defaultValue(134_217_728)
                    ->end()
                    ->integerNode('gc_limit')
                        ->info('Maximum memory usage after which gc_collect_cycles() attempts to free memory: synchronously when the reload threshold may be crossed, deferred to the next event-loop tick otherwise')
                        ->defaultValue(100_663_296)
                    ->end()
                    ->integerNode('gc_cooldown')
                        ->info('Minimum seconds between garbage collection attempts; a collection blocked by the cooldown leaves the reload verdict on the current memory reading')
                        ->defaultValue(60)
                    ->end()
                ->end()
            ->end();
