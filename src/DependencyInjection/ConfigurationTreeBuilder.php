    ...
    private function configureTopLevelPrimitiveNodes(NodeBuilder $node): void
    {
        $node
            ...
            ->integerNode('connection_timeout')
                ->info('Max seconds to wait for a complete request before closing the connection. Protects against slowloris-style attacks where a client sends data very slowly.')
                ->min(1)
                ->defaultValue(120)
                ->end()
            ->integerNode('keepalive_timeout')
                ->info('Max seconds to keep an idle keep-alive connection open before closing it.')
                ->min(1)
                ->defaultValue(30)
                ->end()
